...
cursor.execute(
"CREATE TABLE IF NOT EXISTS SCHOOL_MANAGEMENT (STUDENT_ID INTEGER AUTO_INCREMENT PRIMARY KEY, NAME TEXT, EMAIL TEXT, PHONE_NO TEXT, GENDER TEXT, DOB TEXT, STREAM TEXT)"
)
...
def add_record():
   ...
   if not name or not email or not contact or not gender or not DOB or not stream:
       ...
       try:
           cursor.execute(
           'INSERT INTO SCHOOL_MANAGEMENT (NAME, EMAIL, PHONE_NO, GENDER, DOB, STREAM) VALUES (%s,%s,%s,%s,%s,%s)', (name, email, contact, gender, DOB, stream)
           )
           connector.commit()
...
def remove_record():
   if not tree.selection():
       ...
       cursor.execute('DELETE FROM SCHOOL_MANAGEMENT WHERE STUDENT_ID=%s', (selection[0],))
       connector.commit()
